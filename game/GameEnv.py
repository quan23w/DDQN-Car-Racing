...
# Create a function to get absolute path for assets
...
class Obstacle:
    def __init__(self, x, y, width, height):
        self.image = pygame.image.load(get_asset_path("images/obstacle.png")).convert_alpha()

        # Scale it so it’s not too large. 
        # (You might want it roughly the same width as the car or smaller.)
    ...
    def draw(self, win):
        # Draw the sign. If you don’t have an image, you can use a colored rect.
        if self.image:
...
class Car:
    def __init__(self, x, y):
        ...
        self.original_image = pygame.image.load(get_asset_path("images/car.png")).convert()
...
class RacingEnv:
    ...
    def __init__(self):
        ...
        self.back_image = pygame.image.load(get_asset_path("images/track.png")).convert()
    ...
    def reset(self):
        ...
        self.game_reward = 0
        self.obstacles = [
            Obstacle(20, 200, 30, 30),  # Positioned on the track
            Obstacle(500, 150, 30, 30),  # Positioned on another section of the track
            Obstacle(650, 250, 30, 30),  # Additional obstacle
            Obstacle(800, 520, 30, 30)   # Additional obstacle
        ]
        self.speed_signs = [
        SpeedSign(330, 250, 100, 100, speed_limit=5),
        SpeedSign(400, 480, 100, 100, speed_limit=5)
        ]
    def step(self, action):
        ...
        for obs in self.obstacles:
            if self.car.rect.colliderect(obs.rect):
                reward += PENALTY  # or any custom penalty
                done = True
        for sign in self.speed_signs:
            if self.car.rect.colliderect(sign.rect):
                if abs(self.car.vel) > sign.speed_limit:
                    reward += PENALTY
                    # Optionally, slow the car down.
                    self.car.vel = sign.speed_limit if self.car.vel > 0 else -sign.speed_limit
    ...
    # Draw control indicators, score, etc.
        ...
        pygame.display.update()
class SpeedSign:
    def __init__(self, x, y, width, height, speed_limit):
        self.image = pygame.image.load(get_asset_path("images/speed-limit.png")).convert_alpha()
